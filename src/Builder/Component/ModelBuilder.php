...
use ESGeneration\Builder\Config\ModelConfig;
...
Body;
    ...
    protected function addGetAllMethod(ClassType $phpClass, $keyword = '')
    {
        ...
        $keywords = [];
        if (empty($keyword)) {
            echo "(getAll) 请输入需搜索的关键字".PHP_EOL;
            echo "不需搜索请直接回车，多个用英文逗号分开，勿用空格、中横线、数字，尾部勿带逗号".PHP_EOL;
        ...
        if (!empty($keywords)) {
            foreach ($keywords as $key) {
                $method->addComment("@keyword $key");
            }
        }
        //配置方法参数
        $method->addParameter('page', 1)
            ->setTypeHint('int');
        $method->addParameter('keyword', [])
                ->setTypeHint('array');
        ...
        foreach ($method->getParameters() as $parameter) {
            if ('keyword' == $parameter->getName()) {
                $method->addComment('@param  array $keyword  []');
            } else {
        ...
        $methodBody = '';
        if (!empty($keywords)) {
            $methodBody .= 'if (!empty($keyword)) {' . PHP_EOL . '// 此位置请自行添加检索规则' . PHP_EOL;
            foreach ($keywords as $key) {
                $methodBody .= '
    if (isset($keyword[\'' . $key . '\'])){
        $this->getDb()->where(\'' . $key . '\', \'%\' . $keyword[\'' . $key . '\']. \'%\', \'like\');
    }';
            }
